...
/// concurrent load. This profile's wider values (2s idle_timeout, 0.3s/1.2s retry delays) leave
/// ~0.6s of margin on either side instead.
/// </summary>
...
public sealed class RateLimitIdleInteractionTests(RateLimitIdleInteractionTimersConformanceFixture fixture)
{
    ...
    public Task A_retry_is_not_guest_activity_the_idle_clock_still_closes_the_socket_on_schedule() => fixture.RunAsync(async () =>
    {
        ...
        // (CONFORMANCE_RATE_LIMIT_RETRY_DELAY_SECONDS=0.3, then
        // CONFORMANCE_RATE_LIMIT_SECOND_RETRY_DELAY_SECONDS=1.2 after that, i.e. ~1.5s total),
        // giving a wide, robust gap between the two hypotheses: if retries are correctly NOT
        // guest activity, idle_timeout=2s from the response.create below plus at most one 0.2s
        // sweep pass closes by ~2.0-2.2s; if a retry wrongly touched the idle clock (rtmt.py's
        ...
        // RateLimitRecoveryTests' class doc comment), last activity would reset to ~1.5s and the
        // close would instead land no earlier than ~3.5s. A stopwatch started at the same
        ...
        Assert.True(stopwatch.Elapsed < TimeSpan.FromSeconds(2.8),
            $"Idle close took {stopwatch.Elapsed.TotalSeconds:F2}s after the response.create -- expected " +
            "~2.0-2.2s (idle_timeout plus at most one sweep pass). Anything approaching ~3.5s+ would mean " +
