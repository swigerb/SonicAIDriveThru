...
- **ToolResultDirection.TO_BOTH for order updates**: Successful `update_order` must use `TO_BOTH` (not `TO_CLIENT`). `TO_CLIENT` sends empty string to the AI model — it gets no confirmation the order succeeded and goes silent. `TO_BOTH` sends the order summary JSON to both OpenAI (so the AI can say "anything else?") and the frontend (so the UI updates). This was a conversation-killing bug: ordering exactly 10 burgers (valid) caused dead silence because the success response never reached the AI.
